...
$(document).on("click", ".addObject", function () {
  ...
  $(this)
    .parent()
    .find(".itemsObject:last")
    .append(
      $(this)
        .parent()
        .find(".itemsObject .itemObject:first")[0]
        .outerHTML.replace(/_0/g, "_" + itemObjectLength),
    );
});
...
function changeItemObject(column) {
  ...
  propertyObject.each((i) => {
    object +=
      '"' +
      $(propertyObject[i]).val() +
      '":"' +
      $(valueObject[i]).val().replace(/"/g, '\\"') +
      '",';
  });
...
$(document).on("click", ".addJson", function () {
  ...
  $(this)
    ...
    .append(
      $(this)
        .parent()
        .find(".itemsJson .itemJson:first")[0]
        .outerHTML.replace(/_0/g, "_" + itemJsonLength),
    );
});

/*=============================================
Quitar un objeto en el JSON
=============================================*/
...
/*=============================================
Adicionar un grupo de objetos JSON
=============================================*/
...
$(document).on("click", ".addJsonGroup", function () {
  ...
  $(this)
    ...
    .after(
      $(this)
        .parent()
        .find(".jsonGroup:first")[0]
        .outerHTML.replace(/0_/g, jsonGroupLength + "_"),
    );
});

/*=============================================
Remover un grupo de objetos JSON
=============================================*/
...
function changeItemJson(column) {
  ...
  jsonGroup.each((f) => {
    var propertyJson = $(
      "." + $(jsonGroup[f]).attr("position") + "propertyJson." + column,
    );
    var valueJson = $(
      "." + $(jsonGroup[f]).attr("position") + "valueJson." + column,
    );
...
$(document).on("click", ".myFiles", function () {
  ...
  $("#myFiles").on("shown.bs.modal", function () {
    $(".modal-body").find(".copyLink").append()
      .html(`<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-bar-down" viewBox="0 0 16 16">
